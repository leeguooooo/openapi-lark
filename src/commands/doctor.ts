...
export async function runDoctor(args: DoctorArgs): Promise<number> {
  ...
  if (cfgPath) {
    try {
      ...
      // Warn when the pinned lark-cli range is older than what doctor's auth
      // check needs (1.0.34 added `auth check` subcommand). Old yaml files
      // generated by openapi-lark <= 0.2.1 default to `>=0.1.0` — works but
      // skips scope preflight, costing the user a real-sync round trip to
      // discover missing scopes. Suggest bump.
      const pinned = loaded.config.engines?.larkCli;
      if (pinned && /^>=0\.\d+\.\d+/.test(pinned) && !/^>=1\./.test(pinned)) {
        checks.push({
          name: 'config.engines.larkCli',
          status: 'warn',
          detail:
            `engines.larkCli="${pinned}" is older than what doctor needs for the auth ` +
            `check probe (≥1.0.34). Bump to ">=1.0.34" to get full scope preflight ` +
            `instead of "skipped". Old default came from openapi-lark ≤0.2.1.`,
        });
      }
    } catch (err) {
      checks.push({
  ...
  // Three states (v0.2.2+): ok / warn / fail. Crucially `needs_refresh` with
  // a still-valid refresh token is WARN, not fail — sync will auto-refresh
  // on the next call. v0.2.x previously hard-failed on this, which sent users
  // to `auth login` and sometimes into app-pending-approval dead ends.
  ...
  if (auth.state === 'fail') {
    ...
    checks.push({
      name: 'auth',
      status: auth.state === 'warn' ? 'warn' : 'pass',
      detail:
        auth.state === 'warn'
          ? (auth.reason ?? `tokenStatus=${auth.tokenStatus ?? 'unknown'}`) +
            ` — sync will auto-refresh, no action needed`
          : `tokenStatus=${auth.tokenStatus ?? 'unknown'}, ${auth.scopes.length} scope(s)${expiryInfo}`,
    });
  }
  ...
  const passed = checks.length - failed - warned - skipped;
  // warn does NOT contribute to exit code — it's informational. Only `fail`
  // sets EXIT_ENV, matching user expectation that needs_refresh shouldn't
  // block sync (which auto-refreshes anyway).
  process.stdout.write(
    `\n${passed} ok / ${warned} warning / ${failed} failed / ${skipped} skipped\n`,
  );
