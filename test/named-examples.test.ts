...
describe('namedExamplesForOperation', () => {
  it('content.examples 全部返回，带 summary 与状态码', () => {
    const op = { responses: { '200': { content: { 'application/json': { schema: okSchema, examples: {
      金币: { summary: '普通掉落', description: '一项配了 coin', value: { code: '0', amount: 300 } },
      御守: { value: { code: '0', amount: 0 } },
    } } } } } };
    const ex = namedExamplesForOperation(op);
    expect(ex).toEqual([
      { status: '200', name: '金币', summary: '普通掉落', description: '一项配了 coin', value: { code: '0', amount: 300 } },
      { status: '200', name: '御守', summary: '', description: '', value: { code: '0', amount: 0 } },
    ]);
  });
  it('只有单个 example 时也用作者的，不合成', () => {
    const op = { responses: { '200': { content: { 'application/json': { schema: okSchema, example: { code: '0', amount: 7 } } } } } };
    expect(namedExamplesForOperation(op)).toEqual([{ status: '200', name: '', summary: '', description: '', value: { code: '0', amount: 7 } }]);
  });
...
describe('endpoint 渲染：具名示例每条一节，caption 带标题', async () => {
  const api = {
    ...
    paths: { '/roll': { post: {
      summary: '掷骰', responses: { '200': { description: 'ok', content: { 'application/json': { schema: okSchema, examples: {
        金币: { summary: '金币', description: '普通掉落：金币', value: { code: '0', amount: 300 } },
        御守: { description: '御守 +1', value: { code: '0', amount: 0 } },
      } } } } },
    } } },
  };
  ...
  it('飞书 XML 的代码块 caption 就是小节标题', () => {
    ...
    // 两个响应示例 + 一个请求示例（curl）
    expect((xml.match(/lang="json"/g) || []).length).toBe(2);
  });
});
