...
- **DocxXML 富排版**（仅 endpoint 模式）：endpoint leaf 文档以 Lark DocxXML 推送（`docs +update --doc-format xml`），叠加可正常落地的富 block —
  ...
  - **v0.10 必填列 + 真彩色（post-push 块着色）**：所有字段表（参数 / 响应 Schema / 请求体）的「必填」列单元格 → 必填用「必填」(红字)、可选用 `—`。识别 `true`/`false` 与 `是`/`否`/`required` 取值；仅改该列单元格，列头与其他列不动。**颜色不走导入路径**（那会被剥离），而是 **post-push 块着色 pass**（`src/lark/block-style.ts`）：每个本次实际推送成功的文档，拉取 blocks → 按可插拔规则定位文本元素 → 一次 `docx/v1/documents/{id}/blocks/batch_update` 上色。当前规则 `ruleRequiredCellsRed`：定位字段表「必填」列，对**正文行**（跳过表头）值为「必填」的单元格设 `text_color:1`。best-effort：拉取/更新失败仅告警，不把 push 结果翻成 failed；dry-run / cache-skip 文档不跑（每个推送文档 +1 fetch +1 batch_update）。调色板 1红/2橙/3黄/4绿/5蓝…15灰；`background_color` 同理（留作后续 highlight 规则）。
...
- **枚举值逐值中文说明**（v0.12.0）：枚举值表原来只有「字段 | 取值」两列，取值是 `dice`/`thunder_hit` 这种英文标识，读者得回上面的字段描述里翻。现在读 spec 的 `x-enumDescriptions`（`{取值: 说明}`，OpenAPI 没有标准字段描述单个枚举值，这是社区通行扩展键；`x-enum-descriptions` 连字符写法也认），把表升成三列补上「说明」。只配了部分取值时缺的那格留空不错位；整张表都没配就保持两列。zod 4 用 `.describe(...).meta({ "x-enumDescriptions": {...} })` 即可产出，chanfana 会原样透传。
- **oneOf / anyOf 字段按变体分表**（v0.13.0）：`body: { oneOf: [common, board, pack, cities] }` 这种「形状由 key 决定」的字段，widdershins 原来输出 `*oneOf*` / `*xor*` 标记 + 每张表一行 `*anonymous*`，转飞书后变体表里的行还拼不出完整路径（祖先在上一张表里），读者只看到四堆字段混在一起——aeroplane 的资源块接口就因此把 oneOf 撤成了 `unknown`。现在每个变体单独一张表，标题「**`<父字段路径>` 形状 i/N：<变体的 description>**」，第一列写完整点路径（`data.chunks[].body.cells[].code`），主表那行的类型在变体全是 object 时由 `any` 改成 `object`。写法：zod `z.union([A.describe("key=common 时：…"), B.describe("key=board 时：…")])`，每个变体的 describe 就是它的标题。只处理 oneOf / anyOf；allOf 早在扁平化阶段就摊平了。
- **具名示例**（v0.14.0）：响应 / 请求体的 content 上作者写了 OpenAPI `examples`（具名、可多个）或单个 `example`，就原样用，不再从 schema 合成。响应示例每条一个小节「### 响应示例 (200)：<summary 或键名>」，转飞书后代码块 caption 就是这个标题；请求体示例取第一条。chanfana 写法：`content: { "application/json": { schema, examples: { 金币: { summary: "…", value: {...} } } } }`。只有 schema 时行为不变（合成一条）。
